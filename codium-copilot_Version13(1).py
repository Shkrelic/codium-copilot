...
def update_user_product_json(extension_proposals: Dict[str, List[str]]) -> bool:
    ...
    try:
        ...
        for ext_id, proposals in extension_proposals.items():
            if proposals:
                existing_data['extensionEnabledApiProposals'][ext_id] = proposals
                print_success(f"Configured {len(proposals)} API proposals for {ext_id}", 4)
        ...
        # Add critical proposals that might be missing
        critical_proposals = [
            'languageModelPicker',
        ...
        for ext_id in extension_proposals.keys():
            proposals = existing_data['extensionEnabledApiProposals'][ext_id]
            added_critical = []
...
def find_compatible_version_with_api_check(
    ...
    print_info(f"Will verify API proposal compatibility for each version", 4)
    ...
    for attempt in range(1, INSTALL_RETRY_COUNT + 1):
        try:
            ...
            if 'ScanningExtension' in error_msg:
                print_warning(f"Scanning error occurred, verifying installation...", 4)
                time.sleep(2)
                if is_extension_installed(extension_name):
                    print_success(f"Extension appears to be installed despite error")
                    return True
...
def main() -> int:
    ...
    if len(downloads) != len(EXTENSIONS):
        print_error(f"\nFailed to find a compatible version", 0)
    ...
    config_success = update_user_product_json(extension_proposals)
    ...
    if validation_ok and config_success:
        ...
        print_info(f"✓ API Proposals: Configured", 0)
        print_info(f"✓ Model Selector: Enabled", 0)
        print_info(f"✓ Auto-Completions: Enabled", 0)
        print_info(f"✓ Settings: Updated", 0)
